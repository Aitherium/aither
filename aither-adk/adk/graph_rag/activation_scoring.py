...
The confidence/freshness formulas are self-contained so the scorer is a
standalone, well-tested unit.  All weights/multipliers default to conservative
code constants (start near 1.0, tune from logged score breakdowns) and may be
overridden from an optional YAML file pointed to by the ``ADK_MEMORY_CONFIG``
...
# ═════════════════════════════════════════════════════════════════════════
# DEFAULTS (overridable via the ADK_MEMORY_CONFIG YAML file)
# ═════════════════════════════════════════════════════════════════════════
...
class ScoringConfig:
    ...
    def from_yaml(path: Optional[str] = None) -> "ScoringConfig":
        """Load overrides from a YAML file, or return code defaults.

        ``path`` defaults to the ``ADK_MEMORY_CONFIG`` environment variable.
        If it is unset or the file is missing, the conservative code defaults
        apply (``yaml`` is only imported when a config file actually exists).
        """
        cfg = ScoringConfig()
...
def _default_config_path() -> Optional[str]:
    """Resolve the optional scoring-config path from the environment.

    Returns the value of ``ADK_MEMORY_CONFIG`` if set, else ``None`` so the
    code defaults apply.  No filesystem layout is assumed.
    """
    return os.getenv("ADK_MEMORY_CONFIG") or None
...
class MemoryScorer:
    ...
    def effective_confidence(self, rec: MemoryRecord) -> float:
        ...
        A heavily reinforced, uncontradicted memory gains confidence; a
        contradicted (superseded) one loses it.  A consulted judge that
        rejected caps the result.
        """
